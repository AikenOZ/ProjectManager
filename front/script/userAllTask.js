...
function fetchTasks(page = 1, priority = '') {
    $.ajax({
        url: 'http://127.0.0.1:8000/userAllTask',
        type: 'GET',
               ...
               ` ;
                ...
                let card = `
                    <div class="card mb-3" style="max-width: 18rem;"> 
                        ...
                        <div class="card-footer text-body-secondary"> 
                            <form action="" method="POST" class="updateStatusForm" id="updateStatus_${task.id}">
                                ...
                                ${statusOptions}
                                ${task.status === 'Завершена' ? '<p>Статус: Завершена (изменение невозможно)</p>' : '<button type="submit" class="btn btn-danger">Сохранить статус</button>'}
                            </form> 
                            Приоритет: ${task.priority} 
                        </div> 
                        <div class="commentary">  
                            <p>Оставить комментарий</p> 
                            <button type="button" class="btn btn-danger add-comment-btn" data-bs-toggle="modal" data-bs-target="#commentModal" data-task-id="${task.id}"><img src="../images/icons8-комментарии-48.png" alt="" class="img-comment"></button>
                        </div> 
                    </div>`;
                    ...
                    $(`#updateStatus_${task.id}`).on('submit', function (e) {
                        ...
                        $.ajax({
                            url: `http://127.0.0.1:8000/UserTasksUpdate/${task.id}`,
                            type: 'PATCH',
                            data: $(this).serialize(),
...
function displayPagination(data) {
    ...
    for (let i = 1; i <= data.last_page; i++) {
        const pageItem = `<button class="page-link ${i === currentPage ? 'active' : ''}" data-page="${i}">${i}</button>`;
